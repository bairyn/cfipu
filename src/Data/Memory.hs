...
data Memory = ID { _id_p :: InternalContainer
                 , _id_i :: InternalContainer
                 , id_d  :: InternalContainer
                 }
            | DI { _di_p :: InternalContainer
                 , di_d  :: InternalContainer
                 , _di_i :: InternalContainer
                 }
            | E  { _e_p  :: InternalContainer
...
getInstruction (ID p  i _d) =
    case popFront i of
        ((Just cell), _) -> cell
        ((Nothing),   _) -> getInstruction $ E p i
getInstruction (DI _p _d i) =
    case popFront i of
        ((Just cell), _) -> cell
        ((Nothing),   _) -> defaultCell
getInstruction (E  _p e)    =
    case popFront e of
        ((Just cell), _) -> cell
        ((Nothing),   _) -> defaultCell
...
setData cell mem@(ID _p _i d) = mem{id_d = flip pushFront cell . snd $ popFront d}
setData cell mem@(DI p  d  i)
    | null d    = setData cell $ E p i
    | otherwise = mem{di_d  = flip pushFront cell . snd $ popFront d}
setData cell mem@(E  _p e)   = mem{e_e  = flip pushFront cell . snd $ popFront e}
